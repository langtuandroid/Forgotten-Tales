...
half3 TriplanarNormal(half3 normal, half3 tangent, half3 bTangent, half3 normal_front, half3 normal_side, float3 weights, half3 flipUV)
{
    ...
    {      
        uv[0] += ParallaxOffset(_Mask0, SamplerState_Linear_Repeat, _DiffuseRemapOffset0.w, _DiffuseRemapScale0.w, uv[0], tangentViewDir, _ParallaxAffineStepsTerrain, MipMapLod(0, lod), invert);
        #ifndef _LAYERS_ONE
            uv[1] += ParallaxOffset(_Mask1, SamplerState_Linear_Repeat, _DiffuseRemapOffset1.w, _DiffuseRemapScale1.w, uv[1], tangentViewDir, _ParallaxAffineStepsTerrain, MipMapLod(1, lod), invert);
            #ifndef _LAYERS_TWO
                uv[2] += ParallaxOffset(_Mask2, SamplerState_Linear_Repeat, _DiffuseRemapOffset2.w, _DiffuseRemapScale2.w, uv[2], tangentViewDir, _ParallaxAffineStepsTerrain, MipMapLod(2, lod), invert);
                uv[3] += ParallaxOffset(_Mask3, SamplerState_Linear_Repeat, _DiffuseRemapOffset3.w, _DiffuseRemapScale3.w, uv[3], tangentViewDir, _ParallaxAffineStepsTerrain, MipMapLod(3, lod), invert);
            #endif
        #endif
    }
#endif
...
//=========================================================================================
    ...
    {
        mask = SAMPLE_TEXTURE2D(_Mask0, SamplerState_Linear_Repeat, uv);
    ...
    {
        splat = SAMPLE_TEXTURE2D(_Splat0, sampler_Splat0, uv.xy) * half4(_DiffuseRemapScale0.xyz, 1);
        ...
        half3 SampleNormal(float2 uv, float splat_control, half4 normalScale)
        {
            ...
            half3 nrm = half(0.0);
            nrm = UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal0, SamplerState_Linear_Repeat, uv), normalScale.x);
...
//=========================================================================================
    ...
    void DistantUV(out float2 distantUV[2], float2 uvSplat[2])
    {
        ...
        void SampleMask(out half4 masks[2], half4 hasMask, float2 uv[2])
        {
            ...
            masks[0] = SAMPLE_TEXTURE2D(_Mask0, SamplerState_Linear_Repeat, uv[0]);
            masks[1] = SAMPLE_TEXTURE2D(_Mask1, SamplerState_Linear_Repeat, uv[1]);
    ...
    void SampleSplat(out half4 splat[2], float2 uv[2], half4 defaultAlpha, half4 mask[2])
    {
        ...
        half3 SampleNormal(float2 uv[2], float4 splatControl, half4 normalScale)
        {
            ...
            half3 nrm = half(0.0);
            nrm += splatControl.r * UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal0, SamplerState_Linear_Repeat, uv[0]), normalScale.r);
            nrm += splatControl.g * UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal1, SamplerState_Linear_Repeat, uv[1]), normalScale.g);
    ...
    half HeightSum(half4 mask[2], half4 splat_control)
    {
        ...
        {
            half3 normal = UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal0, SamplerState_Linear_Repeat, uv[0]), normalScale.x);
        ...
        void SampleSplatTOL(out half4 splat[2], half4 noTriplanarSplat[2], float2 uv, half defaultAlpha, float4 splat_control, half4 mask)
        {
            ...
            {
                mask[0] = SAMPLE_TEXTURE2D(_Mask0, SamplerState_Linear_Repeat, uv);
...
//=========================================================================================
    ...
    void DistantUV(out float2 distantUV[4], float2 uvSplat[4])
    {
        ...
        void SampleMask(out half4 masks[4], half4 hasMask, float2 uv[4]) 
        {
            ...
            masks[3] = 0.5h;
                ...
                masks[0] = SAMPLE_TEXTURE2D(_Mask0, SamplerState_Linear_Repeat, uv[0]);
                masks[1] = SAMPLE_TEXTURE2D(_Mask1, SamplerState_Linear_Repeat, uv[1]);
                masks[2] = SAMPLE_TEXTURE2D(_Mask2, SamplerState_Linear_Repeat, uv[2]);
                masks[3] = SAMPLE_TEXTURE2D(_Mask3, SamplerState_Linear_Repeat, uv[3]);
            #endif
    ...
    void SampleSplat(out half4 splat[4], float2 uv[4], half4 defaultAlpha, half4 mask[4])
    {
        ...
        {        
            ...
            nrm += splatControl.r * UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal0, SamplerState_Linear_Repeat, uv[0]), normalScale.r);
            nrm += splatControl.g * UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal1, SamplerState_Linear_Repeat, uv[1]), normalScale.g);
            nrm += splatControl.b * UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal2, SamplerState_Linear_Repeat, uv[2]), normalScale.b);
            nrm += splatControl.a * UnpackNormalScale(SAMPLE_TEXTURE2D(_Normal3, SamplerState_Linear_Repeat, uv[3]), normalScale.a);
