	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float4 temp_output_31_0 = ( _BaseColor * tex2D( _BaseColorMap, uv_BaseColorMap ) );
				float2 uv_DetailAlbedo = packedInput.ase_texcoord5.xy * _DetailAlbedo_ST.xy + _DetailAlbedo_ST.zw;
				...
				float3 tex2DNode6 = UnpackNormalScale( tex2D( _Normal, uv_BaseColorMap ), 1.0f );
				float3 lerpResult20 = lerp( tex2DNode6 , BlendNormal( UnpackNormalScale( tex2D( _DetailNormal, uv_DetailAlbedo ), 1.0f ) , tex2DNode6 ) , tex2D( _DetailMask, uv_BaseColorMap ).r);
				#ifdef _USEDETAILMAPS_ON
				...
				float Smoothness45 = (( tex2D( _SpecularSmoothness, uv_BaseColorMap ) * _SpecSmoothTint )).a;
				
				float AO46 = ( 1.0 - ( ( 1.0 - tex2D( _Occlusion, uv_BaseColorMap ).r ) * _OcclusionAmount ) );
				
				surfaceDescription.BaseColor = BaseColor43.rgb;
				surfaceDescription.Normal = Normal44;
				...
				surfaceDescription.Smoothness = Smoothness45;
				surfaceDescription.Occlusion = AO46;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 temp_output_31_0 = ( _BaseColor * tex2D( _BaseColorMap, uv_BaseColorMap ) );
				float2 uv_DetailAlbedo = packedInput.ase_texcoord2.xy * _DetailAlbedo_ST.xy + _DetailAlbedo_ST.zw;
				#ifdef _USEDETAILMAPS_ON
				float4 staticSwitch30 = ( temp_output_31_0 * ( tex2D( _DetailAlbedo, uv_DetailAlbedo ) * float4( (unity_ColorSpaceDouble).rgb , 0.0 ) ) );
				#else
				float4 staticSwitch30 = temp_output_31_0;
		...
		Pass
		{
			...
			UNITY_SETUP_INSTANCE_ID(packedInput);
				...
				float3 tex2DNode6 = UnpackNormalScale( tex2D( _Normal, uv_BaseColorMap ), 1.0f );
				float2 uv_DetailAlbedo = packedInput.ase_texcoord3.xy * _DetailAlbedo_ST.xy + _DetailAlbedo_ST.zw;
				float3 lerpResult20 = lerp( tex2DNode6 , BlendNormal( UnpackNormalScale( tex2D( _DetailNormal, uv_DetailAlbedo ), 1.0f ) , tex2DNode6 ) , tex2D( _DetailMask, uv_BaseColorMap ).r);
				#ifdef _USEDETAILMAPS_ON
				float3 staticSwitch29 = lerpResult20;
				#else
		...
		Pass
		{
						...
						)
			{
				...
				float4 temp_output_31_0 = ( _BaseColor * tex2D( _BaseColorMap, uv_BaseColorMap ) );
				float2 uv_DetailAlbedo = packedInput.ase_texcoord7.xy * _DetailAlbedo_ST.xy + _DetailAlbedo_ST.zw;
				#ifdef _USEDETAILMAPS_ON
				float4 staticSwitch30 = ( temp_output_31_0 * ( tex2D( _DetailAlbedo, uv_DetailAlbedo ) * float4( (unity_ColorSpaceDouble).rgb , 0.0 ) ) );
				#else
				float4 staticSwitch30 = temp_output_31_0;
...
WireConnection;58;7;49;0
WireConnection;58;8;51;0
ASEEND*/
//CHKSM=3D804E478A07752D73819EE9EC59CE4628B0EBBD